...
class DepartmentService(BaseService):
    ...
    def get_all_departments(self):
        ...
        try:
            ...
            error_message = (
                f"Erreur lors de la récupération des départements : {str(e)}"
            )
            log_error(error_message)
            sentry_sdk.capture_exception(e)
    ...
    def create(self, data):
        ...
        try:
            ...
            log_error(
                f"Erreur de validation lors de la création du département : {str(e)}"
            )
            raise
        except Exception as e:
    ...
    def update(self, department_id, new_data):
        ...
        try:
            ...
            return self.safe_execute(
                lambda: self.repository.update(department, new_data)
            )
        except ValueError as e:
            log_error(
                f"Erreur de validation lors de la mise à jour du département {department_id} : {str(e)}"
            )
            raise
        except Exception as e:
