...
def list_all_departments(user):
    ...
    try:
        ...
        if departments:
            ...
            click.echo("Aucun département trouvé.")
    except Exception as e:
        log_error(f"Erreur lors de la récupération des départements : {e}")
        capture_exception(e)
        click.echo("⚠️ Une erreur est survenue lors de l'affichage des départements.")
    finally:
        session.close()
...
@requires_auth(required_roles=[1])  # Seuls les gestionnaires peuvent créer
def create_department(user):
    ...
    try:
        ...
        capture_exception(e)
        click.echo(f"Erreur lors de la création du département.")
    finally:
        session.close()

@requires_auth(required_roles=[1])  # Seuls les gestionnaires peuvent modifier
def update_department(user):
    """Met à jour un département existant avec un menu interactif."""
...
def delete_department(user):
    """Supprime un département avec confirmation."""
    ...
    try:
        ...
        confirm = click.confirm(f"Voulez-vous vraiment supprimer le département '{department.name}' ?", default=False)
        if confirm:
